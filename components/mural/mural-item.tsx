...
import { ItemGrouper } from "./item-grouper"
...
interface MuralItemProps {
  ...
  groups: Group[]
  onUpdateGroups: (groupId: string, itemIds: string[]) => void
...
export function MuralItem({
  ...
  const handleTagsChange = (itemId: string, tags: string[]) => {
    if (onUpdate) {
      onUpdate(itemId, { tags })
    }
  }
        ...
        >
          ...
          {!isEditing && (
            <CardFooter className="p-3 pt-0 flex flex-wrap gap-2">
              ...
              {/* Controles de etiquetas y grupos */}
              {!readOnly && (
