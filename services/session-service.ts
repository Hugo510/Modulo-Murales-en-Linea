...
// Función para obtener información del dispositivo basada en User-Agent
export function getDeviceInfo(userAgent?: string): {
  ...
  let type = "Desktop";
  if (
    /(android|webos|iphone|ipad|ipod|blackberry|windows phone)/i.test(uaLower)
  ) {
    type = /ipad/i.test(uaLower) ? "Tablet" : "Mobile";
  }

  let name = "Navegador";
  if (uaLower.indexOf("firefox") > -1) name = "Firefox";
  else if (uaLower.indexOf("chrome") > -1) name = "Chrome";
  else if (uaLower.indexOf("safari") > -1) name = "Safari";
  else if (uaLower.indexOf("edge") > -1) name = "Edge";
  else if (uaLower.indexOf("opera") > -1) name = "Opera";

  let os = "Desconocido";
  if (uaLower.indexOf("windows") > -1) os = "Windows";
  else if (uaLower.indexOf("mac") > -1) os = "MacOS";
  else if (uaLower.indexOf("linux") > -1) os = "Linux";
  else if (uaLower.indexOf("android") > -1) os = "Android";
...
export function formatExpiresAt(date: Date): string {
  ...
  if (diffHours < 1)
    return `Expira en ${diffMins} ${diffMins === 1 ? "minuto" : "minutos"}`;
  if (diffHours < 24)
    return `Expira en ${diffHours} ${diffHours === 1 ? "hora" : "horas"}`;
  if (diffDays < 30)
    return `Expira en ${diffDays} ${diffDays === 1 ? "día" : "días"}`;
